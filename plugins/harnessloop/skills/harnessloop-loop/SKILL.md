...
- A `- <label>:` line (`- Feedback:`, `- Review:`/`- Reviewer:`/`- Review verdict:`/`- Review digest:`, `- Acceptance evals:`) written inside a 4-space-indented code block. `parse_feedback` / `parse_review_fields` / `parse_acceptance_eval_declaration` all route through `_uncoded_lines`, which now recognizes CommonMark *fenced* code blocks (`` ``` ``/`~~~`, 0-3 space indent, a closing run at least as long as the opening one, no info string on the close, fail-closed on an unclosed fence through EOF) so that a decision.md quoting one of these fields as a literal example inside a fence can no longer outrank the round's real, unfenced declaration. It does **not** track CommonMark's *other*, indentation-only code-block syntax (4+ leading spaces, no fence markers at all) — that requires knowing the enclosing list-item/blockquote's left margin, not just counting leading spaces on the line itself, and is out of scope for this fix. Registered, not silently missed: 4 空格缩进代码块内的 `- <label>:` 行仍会被解析器读到（当作围栏外的活文本处理）。
