...
- Whether a declared external system's `description` is complete, accurate, or free of anything that reads like a credential. `description` is this file's one free-text field and is validated only for being a string (any content, including empty) — the gate never scans it for secret-shaped content. Secret-shaped text anywhere in this project is this repository's own secret-scanning hook's job, never this gate's; the gate has no equivalent scanner and none is added here.
