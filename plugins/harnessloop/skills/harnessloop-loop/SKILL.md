...
- Whether an already-closed round can be trusted to keep reporting the same violations forever, with nothing inside that round's own directory ever touched again. It cannot, and this is the gate's normal mode of operation, not a residual handful of exceptions: `verify_round(project, round_dir, suffix_index, roots)` takes four parameters, and three of the four — `project`, `suffix_index`, `roots` — are today-layer state, rebuilt fresh by `verify_project` on every single run, before any round is ever inspected; only `round_dir` names the round itself. At least seven distinct mechanisms let today's disk state change what an already-closed round is judged to have done. Six of them (the first six below) require touching nothing inside that round's own directory; the seventh requires editing a byte that *is* inside that round's own directory — just not the byte `decision.md` itself names — which is exactly why it gets separate billing rather than folding into the first (this is at least seven, not exactly seven — it covers `verify_round` and its direct call chain, not every function in this module):
  ...
  - **The round's own files' existence** (`scope_lock.exists()`, `decision.exists()`) is itself re-read every run: a closed round's `scope-lock.md` or `decision.md` deleted today — by accident, by an unrelated cleanup, by anything other than that round's own history — changes that round's violation set on the next run: `missing-scope-lock` appears, or every check gated behind `decision.exists()` (E4, B2a, both RAE declaration checks) silently stops firing for that round, not because it passed but because it is no longer being asked.
