...
- `rounds_predecessor_declared` — loop-predecessor gate (batch 2 of `docs/loop-stop-record-spec-20260728.md`, direction reversed by that spec's Appendix F; see `check_loop_predecessor_declaration`). `decision.md` may optionally declare `- Predecessor: <NNNN>` — the round this one continued from, named by the *successor* rather than the (structurally unwritable, per Appendix F.1) forward `continued: <successor>` the original spec design called for. Checked arithmetic-first, filesystem-last: `<NNNN>` must be exactly four digits (`loop-predecessor-invalid-value` otherwise, fail-closed); its integer value must be strictly less than this round's own, checked with no disk access at all (`loop-predecessor-not-backward` otherwise — catches a forward reference, and a self-reference, identically, since both fail strict `<`); only then is the named round's directory checked for existence under this same goal's `rounds/` (`loop-predecessor-missing`). `rounds_predecessor_declared` counts every round that wrote the field at all, valid or not (an unparsable/forward/missing value is still visible via the violations list, exactly like `acceptance-eval-declaration-unparsable` has no coverage field of its own). Absence is silent — zero-migration, exactly like `- Acceptance evals:`.
